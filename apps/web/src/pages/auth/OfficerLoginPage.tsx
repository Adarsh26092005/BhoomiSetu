...
import { ArrowRight, KeyRound, Mail, AlertCircle, Loader2, UserCheck, ArrowLeft } from 'lucide-react';
...
export function OfficerLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email || !password) {
      setErrorMsg('Please enter both official government email and access password.');
      return;
    }

    try {
      await loginMutation.mutateAsync({ email, password, loginType: 'OFFICER' });
      navigate(from, { replace: true });
    } catch (err: unknown) {
      const message =
        err instanceof Error
          ? err.message
          : 'Authentication failed. Please check your credentials and ensure you are using the Government Officer portal.';
      setErrorMsg(message);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-ink-50 text-ink-900">
      ...
      <div className="bg-ink-900 text-paper px-4 py-2 text-xs flex items-center justify-between border-b border-ink-800">
        ...
        <span className="text-[11px] text-ink-400">Authorized Personnel Only</span>
      </div>
        ...
        <div className="w-full max-w-md bg-paper border border-ink-200 rounded-xl shadow-lg p-6 sm:p-8 space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <div className="inline-block">
              <span className="rounded-full bg-terracotta-50 px-2.5 py-0.5 text-[11px] font-bold text-terracotta-700 border border-terracotta-200">
            ...
            <p className="text-xs text-ink-500">
              Central Ministries, State Authorities & District Collectorates
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-ink-400">NIC / JanSamarth Integrated</span>
              </div>
              <div className="relative">
            ...
            >
              {loginMutation.isPending ? (
                <>
                  ...
                  <span>Verifying Government Credentials...</span>
                </>
              ) : (
                <>
          ...
          <div className="border-t border-ink-200 pt-4 text-center">
            ...
            >
              Fill Demo Officer (Ananya Rao • LAO)
            </Button>
          </div>
