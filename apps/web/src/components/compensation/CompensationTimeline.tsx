import { Activity, CheckCircle2, AlertTriangle, Scale, Clock, Send } from 'lucide-react'
import type { CompensationTimelineEvent } from '@/types'
import { formatDateTime } from '@/lib/format'

interface CompensationTimelineProps {
    timeline: CompensationTimelineEvent[]
}

export function CompensationTimeline({ timeline }: CompensationTimelineProps) {
    const getActionIcon = (action: string) => {
        if (action.includes('DISBURSE') || action.includes('SETTLED') || action.includes('PAID')) {
            return <Send className="h-4 w-4 text-signal-600" />
        }
        if (action.includes('AWARD') || action.includes('APPROVED')) {
            return <CheckCircle2 className="h-4 w-4 text-signal-600" />
        }
        if (action.includes('HOLD') || action.includes('DISPUTE')) {
            return <AlertTriangle className="h-4 w-4 text-rust-600" />
        }
        if (action.includes('ASSESS')) {
            return <Scale className="h-4 w-4 text-terracotta-600" />
        }
        ...
        <div className="rounded-xl border border-ink-200 bg-paper p-5 sm:p-6 shadow-xs space-y-4">
            <div className="flex items-center justify-between border-b border-ink-100 pb-3">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h3 className="text-sm font-bold text-ink-900">Compensation Audit & Settlement Timeline</h3>
                        <p className="text-[11px] text-ink-500">Immutable chronological record of assessment and disbursement events</p>
                    </div>
                </div>
