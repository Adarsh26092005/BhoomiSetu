...
interface CompensationRelatedDocumentsProps {
    projectId: string
    parcelId?: string
}

export function CompensationRelatedDocuments({ projectId, parcelId }: CompensationRelatedDocumentsProps) {
        ...
        <div className="rounded-xl border border-ink-200 bg-paper p-5 sm:p-6 shadow-xs space-y-4">
            <div className="flex items-center justify-between border-b border-ink-100 pb-3">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h3 className="text-sm font-bold text-ink-900">Statutory Award & Title Records</h3>
                        <p className="text-[11px] text-ink-500">Supporting gazette awards, RTC pahani, and bank verification vouchers</p>
                    </div>
                </div>
            ...
            {documents.length === 0 ? (
                <div className="p-6 text-center text-xs text-ink-400 bg-ink-50 rounded-lg border border-ink-100">
                    No statutory documents found specifically linked to this parcel/compensation record.
                </div>
            ) : (
