...
import { GitBranch, ExternalLink, Clock, User, ArrowRight, ShieldCheck } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
        ...
        <div className="space-y-1 md:col-span-2">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-ink-900 text-xs truncate">
              {task.title}
            </span>
          </div>
          ...
          <div className="flex items-center gap-1.5 pt-1 text-[11px] text-ink-600">
            ...
            {task.targetStage && (
              <>
                ...
                <span>
                  Next: <strong>{task.targetStage}</strong>
                </span>
              </>
            )}
          </div>
        ...
        <div className="space-y-3 flex flex-col justify-between border-t md:border-t-0 md:border-l border-ink-100 pt-3 md:pt-0 md:pl-4">
          ...
          {!isPiaUser ? (
            ...
            <div className="flex items-center gap-1.5 text-[11px] text-ink-500 font-medium bg-ink-50 p-2 rounded border border-ink-200">
              <ShieldCheck className="h-3.5 w-3.5 text-terracotta-600 shrink-0" />
              <span>Assigned to Competent Authority for review</span>
            </div>
          )}
        </div>
      </div>
