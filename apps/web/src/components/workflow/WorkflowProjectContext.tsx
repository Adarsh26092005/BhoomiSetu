import { Building2, MapPin, Scale, Layers } from 'lucide-react'
import type { WorkflowTask } from '@/types'
import { useProject } from '@/hooks/use-projects'
import { formatINR, formatArea } from '@/lib/format'
import { StatusBadge } from '@/components/common/StatusBadge'

interface WorkflowProjectContextProps {
    task: WorkflowTask
}

export function WorkflowProjectContext({ task }: WorkflowProjectContextProps) {
            ...
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-4 text-xs">
                <div className="sm:col-span-2">
                    ...
                    <dd className="font-semibold text-ink-900 text-xs mt-0.5">
                        <strong className="font-mono text-terracotta-700">{task.projectCode}</strong> — {task.projectName}
                    </dd>
                </div>
                ...
                <div>
                    ...
                    <dd className="font-semibold text-ink-900 mt-0.5 flex items-center gap-1">
                        ...
                        <span>{project ? `${project.districts.join(', ')}, ${project.state}` : 'Jurisdiction'}</span>
                    </dd>
                </div>

                <div>
                    <dt className="text-ink-400 font-medium text-[11px]">Alignment Area</dt>
                ...
                <div>
                    <dt className="text-ink-400 font-medium text-[11px]">Compensation Assessed</dt>
