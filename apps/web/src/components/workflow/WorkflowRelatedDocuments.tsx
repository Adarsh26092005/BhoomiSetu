...
interface WorkflowRelatedDocumentsProps {
    projectId: string
}

export function WorkflowRelatedDocuments({ projectId }: WorkflowRelatedDocumentsProps) {
        ...
        <div className="rounded-xl border border-ink-200 bg-paper p-5 sm:p-6 shadow-xs space-y-4">
            <div className="flex items-center justify-between border-b border-ink-100 pb-3">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h3 className="text-sm font-bold text-ink-900">Required Statutory Vault Documents</h3>
                        <p className="text-[11px] text-ink-500">Supporting gazettes, surveys, and revenue files for this milestone</p>
                    </div>
                </div>
            ...
            {documents.length === 0 ? (
                <div className="p-6 text-center text-xs text-ink-400 bg-ink-50 rounded-lg border border-ink-100">
                    No statutory documents filed under scheme {projectId} yet.
                </div>
            ) : (
