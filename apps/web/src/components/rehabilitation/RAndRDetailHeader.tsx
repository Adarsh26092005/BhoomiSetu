...
import { ArrowLeft, Landmark, Layers, FileStack, Map, Calendar, HeartHandshake, IndianRupee, Users, Flag } from 'lucide-react'
...
interface RAndRDetailHeaderProps {
    record: RAndRCase
}

export function RAndRDetailHeader({ record }: RAndRDetailHeaderProps) {
    const navigate = useNavigate()

    const totalApproved = record.benefits.reduce((acc, b) => acc + b.approvedValue, 0)
    const totalDelivered = record.benefits.reduce((acc, b) => acc + b.deliveredValue, 0)
        ...
        <div className="rounded-xl border border-ink-200 bg-paper p-5 sm:p-6 shadow-xs space-y-4">
            ...
            <div className="flex items-center justify-between gap-3 border-b border-ink-100 pb-3">
                ...
                >
                    ...
                    <span>Back to R&R Register</span>
                </button>

                <div className="flex items-center gap-2 text-xs text-ink-500 font-mono">
                    <span>Case Ref: <strong className="text-ink-900">{record.id}</strong></span>
                </div>
            </div>
                ...
                <div className="space-y-2">
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="rounded bg-ink-100 px-2 py-0.5 text-[11px] font-mono font-bold text-ink-800 border border-ink-200">
                            Household Ref: {record.household.householdReference}
                        </span>
                    </div>

                    <h1 className="text-xl sm:text-2xl font-black tracking-tight text-ink-900">
                        Rehabilitation & Resettlement Case Dossier
                    </h1>
                    ...
                    <div className="flex flex-wrap items-center gap-y-1 gap-x-4 text-xs text-ink-600">
                        ...
                        <span className="flex items-center gap-1">
                            <HeartHandshake className="h-3.5 w-3.5 text-ink-500" />
                            <span>Plot: {record.surveyNumber} • {record.village}, {record.district}</span>
                        </span>
                        <span>•</span>
                ...
                <div className="flex flex-wrap items-center gap-2 shrink-0">
                    ...
                    {record.possessionId && (
                        <Button
                            ...
                            variant="outline"
                            size="sm"
                            onClick={() => navigate(ROUTES.possessionDetail(record.possessionId!))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 pt-3 border-t border-ink-100 text-xs">
                <div className="rounded-lg bg-ink-50 p-3 border border-ink-200 flex items-center justify-between">
                    <div>
                        <span className="text-[11px] text-ink-500 block font-medium">Household Members</span>
                        <span className="font-mono font-bold text-sm text-ink-900 block">
                            {record.household.familySize} Persons ({record.household.affectedMembers} Displaced)
                        </span>
                    </div>
                ...
                <div className="rounded-lg bg-ink-50 p-3 border border-ink-200 flex items-center justify-between">
                    <div>
                        <span className="text-[11px] text-ink-500 block font-medium">Defined Entitlements</span>
                        <span className="font-mono font-bold text-sm text-ink-900 block">
                            {record.entitlements.length} Statutory Packages
                        </span>
                    </div>
                    <HeartHandshake className="h-5 w-5 text-ink-400" />
                </div>

                <div className="rounded-lg bg-signal-50/60 p-3 border border-signal-200 flex items-center justify-between">
                    <div>
                        <span className="text-[11px] text-signal-700 block font-medium">Assistance Delivered</span>
                        <span className="font-mono font-bold text-sm text-signal-900 block">
                            {formatINR(totalDelivered)} ({totalApproved > 0 ? Math.round((totalDelivered / totalApproved) * 100) : 0}%)
                        </span>
                    </div>
                ...
                <div className="rounded-lg bg-ink-50 p-3 border border-ink-200 flex items-center justify-between">
                    <div>
                        <span className="text-[11px] text-ink-500 block font-medium">Relocation Target</span>
                        <span className="font-mono font-bold text-sm text-ink-900 block">
                            {record.relocationDate ? formatDate(record.relocationDate) : record.relocationRequired ? 'Scheduled' : 'In-situ (No Shifting)'}
                        </span>
                    </div>
